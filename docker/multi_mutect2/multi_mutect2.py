...
def main():
    ...
    log_file = "{}.{}.multi_mutect2.log".format(os.path.basename(tumor), os.path.basename(normal))
    ...
    outputs = multi_commands(mutect2_cmds, threads, logger)
    if any(x != 0 for x in outputs):
        logger.info('Failed multi_mutect2')
    else:
